...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "zenmux": {
        // ZenMux exposes /models; avoid probing chat because free quota/model scopes may rate-limit.
        const res = await fetchWithConnectionProxy("https://zenmux.ai/api/v1/models", {
          method: "GET",
          headers: { Authorization: `Bearer ${connection.apiKey}`, "content-type": "application/json" },
        }, effectiveProxy);
        const valid = res.status !== 401 && res.status !== 403;
      ...
      case "opencode-go": {
        const res = await fetchWithConnectionProxy("https://opencode.ai/zen/go/v1/chat/completions", {
          method: "POST",
          headers: { Authorization: `Bearer ${connection.apiKey}`, "content-type": "application/json" },
          body: JSON.stringify({ model: "kimi-k2.7-code", messages: [{ role: "user", content: "test" }], max_tokens: 1, stream: false }),
        }, effectiveProxy);
        const valid = res.status !== 401 && res.status !== 403;
        if (valid) return { valid: true, error: null };
        let detail = "Invalid OpenCode Go API key";
        try {
          const text = await res.text();
          const data = text ? JSON.parse(text) : null;
          detail = data?.error?.message || data?.message || detail;
      ...
      case "llm7": {
        const res = await fetchWithConnectionProxy("https://api.llm7.io/v1/models", { headers: { Authorization: `Bearer ${connection.apiKey}` } }, effectiveProxy);
        const valid = res.status !== 401 && res.status !== 403;
        return { valid, error: valid ? null : "Invalid API key" };
      }
      case "cline-apikey": {
      ...
      case "ocenza": {
        const res = await fetchWithConnectionProxy("https://ocenza.com/v1/models", {
          method: "GET",
          headers: { Authorization: `Bearer ${connection.apiKey}`, "content-type": "application/json" },
        }, effectiveProxy);
        const valid = res.status !== 401 && res.status !== 403;
      ...
      case "gmi":
      case "gmi-cloud": {
        const res = await fetchWithConnectionProxy("https://api.gmi-serving.com/v1/chat/completions", {
          method: "POST",
          headers: { Authorization: `Bearer ${connection.apiKey}`, "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      case "vikey":
      case "vk": {
        const res = await fetchWithConnectionProxy("https://api.vikey.ai/v1/chat/completions", {
          method: "POST",
          headers: { Authorization: `Bearer ${connection.apiKey}`, "Content-Type": "application/json" },
          body: JSON.stringify({
            model: "vikey/vclaw",
      ...
      case "qwen-cloud":
      case "qc": {
        const res = await fetchWithConnectionProxy("https://dashscope-intl.aliyuncs.com/compatible-mode/v1/chat/completions", {
          method: "POST",
          headers: { Authorization: `Bearer ${connection.apiKey}`, "Content-Type": "application/json" },
          body: JSON.stringify({
            model: "qwen3.7-max",
