...
export async function clearUsageData({ scope = "all", vacuum = true } = {}) {
  const db = await getAdapter();
  const normalizedScope = ["all", "usage", "details"].includes(scope) ? scope : "all";

  const before = {
    usageHistory: db.get(`SELECT COUNT(*) as c FROM usageHistory`)?.c || 0,
    usageDaily: db.get(`SELECT COUNT(*) as c FROM usageDaily`)?.c || 0,
    requestDetails: db.get(`SELECT COUNT(*) as c FROM requestDetails`)?.c || 0,
  };

  db.transaction(() => {
    if (normalizedScope === "all" || normalizedScope === "usage") {
      db.run(`DELETE FROM usageHistory`);
      db.run(`DELETE FROM usageDaily`);
      db.run(`DELETE FROM _meta WHERE key = 'totalRequestsLifetime'`);
    }
    if (normalizedScope === "all" || normalizedScope === "details") {
  ...
  if (vacuum) {
    try { db.exec(`VACUUM`); } catch (e) { console.warn("[usageRepo] VACUUM skipped:", e.message); }
    try { db.checkpoint?.(); } catch {}
  }
  ...
  return {
    ...
    after,
    deleted: {
      usageHistory: before.usageHistory - after.usageHistory,
      usageDaily: before.usageDaily - after.usageDaily,
      requestDetails: before.requestDetails - after.requestDetails,
    },
  };
}
