...
export async function handleFakeStreamingFromJson({ providerResponse, provider, model, requestStartTime, connectionId, apiKey, clientRawRequest, body, translatedBody, finalBody, onRequestSuccess, trackDone, appendLog }) {
  ...
  if (provider === "anuma" || provider === "zyloo") {
    if (provider === "anuma") responseBody = normalizeAnumaTextToolCall(normalizeAnumaResponsesJson(responseBody));
    ...
    if (!content && !hasToolCalls && choice?.finish_reason !== "length") {
      ...
      return { success: false, response: new Response(JSON.stringify({ error: { message: `${label} returned empty assistant content; retry/fallback instead of treating this as a successful no-reply.` } }), { status: 502, headers: { "Content-Type": "application/json" } }) };
    }
  }
